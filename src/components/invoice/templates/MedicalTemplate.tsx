...
export const MedicalTemplate = ({ invoice, settings }: TemplateProps) => {
  ...
  return (
    <div className="invoice-wrap px-4 py-4 bg-white font-sans text-[11px] border border-zinc-300 flex flex-col">
      {/* Header */}
      ...
      {/* Items Table */}
      <table className="w-full border-collapse border border-zinc-400 mb-2">
        <thead>
          <tr className="border-b border-zinc-400 bg-zinc-50">
            <th className="border-r border-zinc-400 py-[2px] px-1 w-5 text-center">Sr.</th>
            <th className="border-r border-zinc-400 py-[2px] px-1 text-left">Product</th>
            <th className="border-r border-zinc-400 py-[2px] px-1 w-10 text-center">Unit</th>
            <th className="border-r border-zinc-400 py-[2px] px-1 w-14 text-center">Batch</th>
            <th className="border-r border-zinc-400 py-[2px] px-1 w-12 text-center">Expiry</th>
            <th className="border-r border-zinc-400 py-[2px] px-1 w-8 text-center">Qty</th>
            <th className="border-r border-zinc-400 py-[2px] px-1 w-16 text-right">MRP</th>
            <th className="py-[2px] px-1 w-16 text-right">Amount</th>
          </tr>
        </thead>
        <tbody>
          {invoice.items.map((item, i) => (
            <tr key={i} className="border-b border-zinc-100">
              ...
              <td className="border-r border-zinc-400 py-[1px] px-1 text-center text-[9px]">{item.unit || '-'}</td>
              <td className="border-r border-zinc-400 py-[1px] px-1 text-center text-[9px]">{(item as any).batchNo || '-'}</td>
              <td className="border-r border-zinc-400 py-[1px] px-1 text-center text-[9px]">{formatExpiry((item as any).expiryDate, (item as any).expiryMode)}</td>
              <td className="border-r border-zinc-400 py-[1px] px-1 text-center">{item.quantity}</td>
              <td className="border-r border-zinc-400 py-[1px] px-1 text-right">{item.unitPrice.toFixed(2)}</td>
              <td className="py-[1px] px-1 text-right font-bold">{item.total.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="bg-zinc-50 font-bold border-t border-zinc-400">
            ...
            <td className="border-r border-zinc-400 py-[2px] px-1 text-center">{invoice.items.reduce((s, i) => s + i.quantity, 0)}</td>
            <td className="border-r border-zinc-400 py-[2px] px-1"></td>
            <td className="py-[2px] px-1 text-right">{subtotal.toFixed(2)}</td>
          </tr>
        </tfoot>
      </table>
